...
{
	if (!block->ret_type || block->ret_type->name == "n")
	{
	...
	{
		buffer.emplace_back("LOAD_GLOBAL", Block::getName(block, "flowerror"));
	...
	try
	{
		...
		if (auto pASTAssignStmt = dynamic_cast<ASTAssignStmt*>(node); pASTAssignStmt) // TODO: type check
		{
			...
			if (auto ppid = std::get_if<std::shared_ptr<ASTIdentifier>>(&pASTAssignStmt->left); ppid)
			{
				...
				{
					if (rtype->name != block->ret_type->name)
						generateConvert(block, rtype.get(), block->ret_type.get(), buffer);
					buffer.emplace_back("RETURN_VALUE");
				}
				else
				{
		...
		{
			auto type = generateInstruction(block, pASTCallStmt->identifier.get(), buffer);

			auto functype = dynamic_cast<Block::FunctionType*>(type.get());

			if (!functype)
			{
				throw std::logic_error(type->name + " is not callable");
			}
		...
		if (auto pASTBinaryOp = dynamic_cast<ASTBinaryOp*>(node); pASTBinaryOp)
		{
			...
			if (pASTBinaryOp->op == pASTBinaryOp->SLASH)
				ret = std::make_shared<Block::RealType>();
